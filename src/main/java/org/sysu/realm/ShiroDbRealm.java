...
public class ShiroDbRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		User user = userService.selectByLoginID(token.getUsername());
		System.out.println(user);
		CipherUtil cipher = new CipherUtil();//MD5����
		if (user != null) {
			return new SimpleAuthenticationInfo(user.getLoginId(), cipher.generatePassword(user.getPassword()), getName());
		}else{
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
	    ...
	    User user = this.userService.selectByLoginID(username);
	    List<UserRole> urList = this.userRoleService.selectByLoginId(user.getId());
	    for(UserRole ur : urList) {
	    	List<RolePermission> rpList = this.rolePermissionService.selectByRoleId(ur.getRoleId());
	    	if(rpList != null) {
	    		for(int i = 0; i < rpList.size(); i++) {
	    			RolePermission rp = rpList.get(i);
	    			roleNames.add(rp.getRoleName());
	    			permissions.add(rp.getPermissionName());
	    		}
	    	}
	    }
